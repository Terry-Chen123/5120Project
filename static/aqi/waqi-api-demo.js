    ...
    function search(keyword, output) {
      ...
      $.getJSON(
        "//api.waqi.info/search/?token=7738c648d0b182560761a45befc6f0a2947006bc&keyword=" + keyword,
        function (result) {
          var info = token() == "demo" ? "" : "";
          output.html("<h2>Search results " + info + ":</h2>");
          ...
          output.append(
            $("<div/>").html("Click on any of the location to see the detailled AQI")
          );
    ...
    function showStation(station, output) {
      ...
      $.getJSON(
        "//api.waqi.info/feed/@" + station.uid + "/?token=7738c648d0b182560761a45befc6f0a2947006bc&keyword",
        function (result) {
          output.html("<h2>Pollutants & Weather conditions:</h2>");
          if (!result || result.status != "ok") {
            output.append("Sorry, something wrong happend: ");
          ...
          output.append(
            $("<div style='color: limegreen'>").html(
              "Location: " + result.data.city.name + " on " + result.data.time.s
            )
          );
    ...
    //add color of each air quality index
